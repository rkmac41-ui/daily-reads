...
def ask_claude_via_subscription(prompt):
    ...
    try:
        ...
        print("  ! claude CLI not found on PATH (is it installed in this workflow?)")
        return None
...
def ask_claude(prompt, max_tokens=3000):
    ...
    if os.environ.get("CLAUDE_CODE_OAUTH_TOKEN"):
        ...
        if out:
            return out
        print("  ! subscription CLI route failed, trying API key fallback if set")
    return ask_claude_via_api(prompt, max_tokens=max_tokens)
